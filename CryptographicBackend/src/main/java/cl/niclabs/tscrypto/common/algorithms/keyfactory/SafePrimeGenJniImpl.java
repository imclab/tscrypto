...
class SafePrimeGenJniImpl implements SafePrimeGen {
	...
	private static final Logger log = Logger.getLogger(SafePrimeGenJniImpl.class
			.getName());

	/**
	 * Default for the certainty.
	 * 
	 * This value tells a BigInteger constructor the probability of errors for
	 * the constructor internal prime test in the form 2^-certainty, or you can
	 * say the probability a prime number exceeds 1-1/2^certainty.
	 *
	 */
	...
	public static BigInteger generateStrongPrime(final int bitLength,
		 ...
		 * 
		 * 1. <code> random </code> the PRNG 2. <code> certainty </code> the
		 * default for the certainty.
		 * 
		 * Note: We use no expliziet this reference in this method. That makes
		 * the code clear and readable.
		 * 
		 * The method is a private helper method of <code> generateKeyPair()
		 * </code> to generate random strong primes with a given <code>
		 * bitLength </code> .
		 * 
		 * This method uses DEBUG and INFO log4j statements. DEBUG means a lot
		 * of messages. INFO means less highlevel infos. Use log4j.properties
		...
		if (log.isLoggable(Level.FINEST))
			log.finest("Have set <i0BitLengthTargetValue="
		...
		if (log.isLoggable(Level.FINEST))
			log.finest("Have set <j0BitLengthTargetValue="
					+ j0BitLengthTargetValue + ">.");
		...
		BigInteger i = i0;
		if (log.isLoggable(Level.FINEST))
			log.finest("Have set <i=" + i
					+ "> with <i0> as start value for loop 1.");
		...
		if (log.isLoggable(Level.FINER))
			log.finer("Have found <r> as probable prime in <2 * " + i
					+ " * t + 1> in "
					+ "loop 1, with bit length <r.bitLength()=" + r.bitLength()
					+ ">, where <r=" + r + ">.");
		...
		outerloop: do {
			...
			// ////
			final int sBitLength = bitLength - r.bitLength()
					- j0BitLengthTargetValue;
			if (log.isLoggable(Level.FINEST))
				log
			...
			c = JniSignWrapper.modPow(s, b, r); // c = s^b mod r --> s^(r-2) mod r
			d = c.multiply(a); // d = c * a --> 2 * (s^(r-2) mod r) * s
			p0 = d.subtract(ThreshUtil.ONE); // p0 = d - 1 --> 2 * (s^(r-2) mod
												// r) *
			// s - 1
			if (log.isLoggable(Level.FINEST))
				log
			...
			do {
				...
				if (log.isLoggable(Level.FINEST))
					log.finest("Try to find <p> in <p0 + 2 * " + j
							+ " * r * s> in loop 3, "
							+ "with bit length <p.bitLength()=" + p.bitLength()
							+ ">, " + "where <p=" + p + ">.");
			...
			if (log.isLoggable(Level.FINER))
				log
						.finer("Have found <p> as probable strong prime <p0 + 2 * "
								+ j
		...
		if (log.isLoggable(Level.FINEST))
			log.finest("Have left loop 2, because <p.bitLength()="
					+ p.bitLength() + "> == <bitLength=" + bitLength + ">");
