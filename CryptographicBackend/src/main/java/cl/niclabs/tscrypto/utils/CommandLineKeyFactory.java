...
public class CommandLineKeyFactory {
	...
	private void run() {
		// print basic info
		TSLogger.keyDealer.info("Generating keys with:");
		TSLogger.keyDealer.info("k=" + k);
		TSLogger.keyDealer.info("l=" + l);
		TSLogger.keyDealer.info("keysize=" + keysize);
		for (int id=0; id<rsaKeys.length; id++) {
			TSLogger.keyDealer.info("rsa key file" + id + "=" + rsaKeys[id]);
		}

		// generate keys
		KeyInfo key = new KeyFactoryImpl().generateKeys(keysize, k, l);
		TSLogger.keyDealer.info("Done... writting key with alias " + key.getKeyMetaInfo().getAlias());
